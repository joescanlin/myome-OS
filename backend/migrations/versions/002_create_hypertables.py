...
def downgrade() -> None:
    ...
    op.execute("SELECT remove_compression_policy('heart_rate_readings', if_exists => TRUE);")
    op.execute("SELECT remove_compression_policy('glucose_readings', if_exists => TRUE);")

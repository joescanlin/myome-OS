...
class HealthProfile(Base, UUIDMixin, TimestampMixin):
    ...
    ethnicity: Mapped[Optional[list[str]]] = mapped_column(
        ARRAY(String),
        nullable=True,
    )
    ...
    alcohol_frequency: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)
    exercise_frequency: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)
    diet_type: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)
    
    # Sleep baseline
    typical_sleep_hours: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    typical_bedtime: Mapped[Optional[str]] = mapped_column(String(10), nullable=True)
    typical_wake_time: Mapped[Optional[str]] = mapped_column(String(10), nullable=True)

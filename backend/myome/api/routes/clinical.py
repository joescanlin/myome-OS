...
async def generate_physician_report(
    user: CurrentUser,
    session: DbSession,
    report_date: Optional[datetime] = Query(default=None),
...
async def get_fhir_bundle(
    ...
    include_hr: bool = Query(default=True, description="Include heart rate observations"),
    include_glucose: bool = Query(default=True, description="Include glucose observations"),
    ...
    # Add patient
    resources.append(fhir.create_patient({
        "first_name": user.first_name,
        "last_name": user.last_name,
        "date_of_birth": user.date_of_birth.isoformat() if user.date_of_birth else None,
        "biological_sex": user.biological_sex,
    ...
    if include_hr:
        ...
        for idx, row in hr_df.iterrows():
            if not row.isna().all():
                resources.append(fhir.create_heart_rate_observation(
    ...
    if include_glucose:
        ...
        for idx, row in glucose_df.iterrows():
            if not row.isna().all():
                resources.append(fhir.create_glucose_observation(
    ...
    if include_hrv:
        ...
        for idx, row in hrv_df.iterrows():
            if 'sdnn_ms' in row and not row.isna().all():
                resources.append(fhir.create_hrv_observation(
                    float(row['sdnn_ms']),
                    idx.to_pydatetime() if hasattr(idx, 'to_pydatetime') else idx,
                ))
...
async def get_fhir_diagnostic_report(
    ...
    if cardio.get("resting_heart_rate"):
        ...
        observations.append(fhir_gen.create_heart_rate_observation(
            int(rhr.get("average", 0)),
            datetime.now(timezone.utc),
        ))
    
    if cardio.get("hrv_analysis") and cardio["hrv_analysis"].get("average_sdnn"):
        observations.append(fhir_gen.create_hrv_observation(
    ...
    if metabolic.get("glucose_metrics"):
        ...
        observations.append(fhir_gen.create_glucose_observation(
...
async def get_fhir_observations(
    ...
    if observation_type == "heart-rate":
        ...
        for idx, row in df.head(limit).iterrows():
            if not row.isna().all():
                observations.append(fhir.create_heart_rate_observation(
                    int(row['heart_rate_bpm']),
                    idx.to_pydatetime() if hasattr(idx, 'to_pydatetime') else idx,
                ))
        ...
        for idx, row in df.head(limit).iterrows():
            if not row.isna().all():
                observations.append(fhir.create_glucose_observation(
                    float(row['glucose_mg_dl']),
                    idx.to_pydatetime() if hasattr(idx, 'to_pydatetime') else idx,
                ))
        ...
        for idx, row in df.head(limit).iterrows():
            if 'sdnn_ms' in row and not row.isna().all():
                observations.append(fhir.create_hrv_observation(
                    float(row['sdnn_ms']),
                    idx.to_pydatetime() if hasattr(idx, 'to_pydatetime') else idx,
                ))

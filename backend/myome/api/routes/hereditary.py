...
from myome.hereditary.models import FamilyMember, FamilyDocument, WatchlistItem
...
async def create_family_member(
    ...
    fm = FamilyMember(
        ...
        age_at_death=member.death_year - member.birth_year if member.death_year and member.birth_year else None,
        notes=member.notes,
        data_source="manual",
    )
...
async def upload_family_document(
    ...
    age_at_document: Optional[int] = Query(default=None),
    document_text: str = Query(..., description="Extracted text from document (OCR if needed)"),
) -> dict:
    """
    ...
    extracted_data = processor.convert_to_family_member_data(

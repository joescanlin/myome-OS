...
class PhysicianReportGenerator:
    ...
    async def generate_report(
        ...
        report = {
            ...
            "executive_summary": await self._generate_executive_summary(start_date, report_date),
            "detailed_analysis": await self._generate_detailed_analysis(start_date, report_date),
            "risk_assessment": await self._generate_risk_assessment(start_date, report_date),
            "recommendations": await self._generate_recommendations(start_date, report_date),
        }
    ...
    async def _generate_executive_summary(
        ...
        critical_alerts = [a for a in analysis.get('alerts', []) if a.get('priority') == 'critical']
        high_alerts = [a for a in analysis.get('alerts', []) if a.get('priority') == 'high']
    ...
    async def _analyze_cardiovascular(self, start: datetime, end: datetime) -> dict:
        ...
        if not hrv_df.empty and 'sdnn_ms' in hrv_df.columns:
            current_hrv = hrv_df['sdnn_ms'].iloc[-1] if not hrv_df['sdnn_ms'].isna().iloc[-1] else None
            avg_hrv = hrv_df['sdnn_ms'].mean()
            
            result["hrv_analysis"] = {
                "current_sdnn": float(current_hrv) if current_hrv else None,
                "average_sdnn": float(avg_hrv) if not hrv_df['sdnn_ms'].isna().all() else None,
    ...
    async def _analyze_metabolic(self, start: datetime, end: datetime) -> dict:
        ...
        if not glucose_df.empty:
            ...
            result["time_in_range"] = {
                ...
                "target": ">90%",
                "status": "good" if tir >= 90 else "needs_improvement" if tir >= 70 else "concerning",
            }
    ...
    async def _analyze_sleep(self, start: datetime, end: datetime) -> dict:
        ...
        if not sleep_df.empty:
            ...
            result["average_duration"] = {
                "minutes": float(avg_duration) if hasattr(avg_duration, '__float__') else avg_duration,
                "hours": float(avg_duration / 60) if hasattr(avg_duration, '__float__') else avg_duration / 60,
                "target": "7-9 hours",
            ...
            result["sleep_architecture"] = {
                ...
                "deep_pct": float((avg_deep / avg_duration) * 100) if avg_deep and avg_duration else None,
                "rem_pct": float((avg_rem / avg_duration) * 100) if avg_rem and avg_duration else None,
            }
            
            result["efficiency"] = {
                "average_pct": float(avg_efficiency) if avg_efficiency else None,
                "target": ">85%",
                "status": "good" if avg_efficiency and avg_efficiency >= 85 else "needs_improvement",
            }
    ...
    async def _generate_recommendations(self, start: datetime, end: datetime) -> list[dict]:
    ...
    async def _get_top_correlations(self, start: datetime, end: datetime) -> list[dict]:
        ...
        try:
            correlations = await self.analytics.correlation_engine.discover_all_correlations(

...
class AnomalyDetector:
        ...
        for timestamp, value in data.items():
            ...
            if 'critical_low' in thresholds and value < thresholds['critical_low']:
                anomalies.append(Anomaly(
                    ...
                    value=float(value),
                    expected_range=(thresholds['critical_low'], thresholds.get('critical_high', float('inf'))),
                    deviation_score=abs(value - thresholds['critical_low']) / thresholds['critical_low'],
                ...
                anomalies.append(Anomaly(
                    ...
                    priority=AlertPriority.CRITICAL,
                    value=float(value),
                    expected_range=(thresholds.get('critical_low', 0), thresholds['critical_high']),
                    deviation_score=(value - thresholds['critical_high']) / thresholds['critical_high'],
            ...
            # Check warning thresholds
                ...
                anomalies.append(Anomaly(
                    ...
                    value=float(value),
                    expected_range=(thresholds['low'], thresholds.get('high', float('inf'))),
                    deviation_score=abs(value - thresholds['low']) / thresholds['low'],
                ...
                anomalies.append(Anomaly(
                    ...
                    deviation_score=(value - thresholds['high']) / thresholds['high'],
    ...
    def _detect_statistical_outliers(
        ...
        rolling_mean = data.rolling(window=self.window_size, min_periods=self.window_size // 2).mean()
        rolling_std = data.rolling(window=self.window_size, min_periods=self.window_size // 2).std()
    ...
    def _detect_level_shifts(
        ...
        for i in range(self.window_size, len(values) - self.window_size, self.window_size // 2):
            ...
            if abs(percent_change) > min_shift_percent:
                ...
                if p_value < 0.01:  # Significant shift
                    ...
                    anomalies.append(Anomaly(
                        ...
                        expected_range=(float(baseline_mean - 2*baseline_std), float(baseline_mean + 2*baseline_std)),

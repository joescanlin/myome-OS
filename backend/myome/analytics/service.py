...
from myome.analytics.alerts.manager import AlertManager, Alert
...
class AnalyticsService:
    ...
    async def run_daily_analysis(
        ...
        logger.info(f"Daily analysis complete for user {self.user_id}: "
    ...
    async def _compute_daily_summary(
        ...
        if not hrv_df.empty:
            summary['hrv'] = {
                'sdnn_mean': float(hrv_df['sdnn_ms'].mean()) if 'sdnn_ms' in hrv_df else None,
                'rmssd_mean': float(hrv_df['rmssd_ms'].mean()) if 'rmssd_ms' in hrv_df else None,
            }
    ...
    async def get_health_score(
        ...
        if not glucose_df.empty:
            cv = glucose_df['glucose_mg_dl'].std() / glucose_df['glucose_mg_dl'].mean() * 100
            tir = ((glucose_df['glucose_mg_dl'] >= 70) & 
                   (glucose_df['glucose_mg_dl'] <= 180)).mean() * 100

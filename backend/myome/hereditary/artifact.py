...
class HereditaryArtifact:
    ...
    async def generate(
        ...
        self._data = {
            ...
            "family_history": self._generate_family_history(
                family_members, privacy_settings
            ),
            "disease_events": await self._generate_disease_events(privacy_settings),
            "environmental_lifestyle": await self._generate_lifestyle_section(
                privacy_settings
            ),
            "interpretation_for_descendants": await self._generate_interpretations(
                privacy_settings
            ),
            "privacy_settings": {
    ...
    async def _generate_biomarker_trajectories(
        ...
        try:
            hr_df = await self.loader.load_heart_rate(
                start_date, end_date, resample="1M"
            )
            if not hr_df.empty:
                trajectories["resting_heart_rate"] = {
                    ...
                    "summary_statistics": self._compute_trajectory_stats(
                        hr_df["heart_rate_bpm"]
                    ),
                }
        ...
        # Glucose trajectory
        try:
            glucose_df = await self.loader.load_glucose(
                start_date, end_date, resample="1M"
            )
...
class ArtifactReader:
    ...
    @staticmethod
    def load(path: str, password: str | None = None) -> dict:
    ...
    def verify_signature(artifact: dict, public_key: str) -> bool:
        ...
        computed = hashlib.sha256((data_str + public_key).encode()).hexdigest()
